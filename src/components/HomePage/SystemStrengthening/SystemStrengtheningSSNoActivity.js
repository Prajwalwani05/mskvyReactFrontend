...
import SelectActionCard from "./Cards_second";
...
const SystemStrengtheningSSNoActivity = () => {
  ...
  const fetchData = async (selectedSpvName = null) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${apiUrl}/api/SystemStrengtheningSSNoStatus`,
        {
          params: {
            SS_No: decryptedObject.SS_No,
            mskvyType: decryptedObject.mskvyType,
            Status: decryptedObject.Status,
            sfile_id: 1,
          },
        }
      );
    ...
    <>
     {
      ...
      (substationData?.Model?.systemStrengtheningSS_NoDetailLists
        ...
        >
          ...
          <Box p={1} pb={2}>
            <Box pb={2}>
              <SelectActionCard data={substationData?.Model?.systemStrengtheningSSCardDetailLists}/>
            </Box>
            <Box sx={{ width: "100%" }}>
...
export default SystemStrengtheningSSNoActivity;
