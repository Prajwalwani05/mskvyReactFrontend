...
const SystemStrengtheningSubstationActivityStatus = () => {
  ...
  const { data, chartData } = location.state || {};
  ...
  useEffect(() => {
    ...
    let newType = "E"; // Default value
    if (chartData?.MSKVY_Type === "S/S Identified Under MSKVY 2.0") {
      newType = "A";
    } else if (
      chartData?.MSKVY_Type === "S/S where LOA Awarded under MSKVY 2.0"
    ) {
      newType = "C";
    }

    setType(newType);
  }, [chartData]); // Runs only when chartData changes
  useEffect(() => {
    if (type) {
      fetchData();
    }
  ...
  const fetchData = async (selectedSpvName = null) => {
    ...
    try {
      const response = await axios.get(
        `${apiUrl}/api/SystemStrengtheningSubstationActivityStaus`,
        {
          params: {
            mskvyType: type,
            Status: status,
      ...
      if (response.status === 200) {
        ...
        console.log(
          "SystemStrengtheningSubstationActivityStaus: ",
          response.data
        );
      } else {
  ...
  return (
    <>
      {loading ? (
        <Loader />
      ) : substationData?.Model?.systemStrengtheningSubstationActivityStausLists
          ?.length > 0 ? (
        <Box
          sx={{
            backgroundColor: "#FFF",
        ...
        >
          ...
          >
            ...
            ></Box>
            <h2 style={{ margin: "15px 0" }}>System Strengthening -</h2>
            <Typography style={{ marginLeft: "0px" }}>
              {chartData?.MSKVY_Type} -{" "}
            </Typography>
            <Typography style={{ marginLeft: "0px" }}>
              <Box
                sx={{
                ...
                >
                  <TableHead>
                    <TableRow>
                      ...
                      <StyledTableCell>Taluka Name</StyledTableCell>
                  ...
                  <TableBody>
                    {substationData?.Model?.systemStrengtheningSubstationActivityStausLists?.map(
                      (element, index) => {
                        const SS_No = element.SS_NO;
                        const sfile_id = "1";
                        const mskvyType = type;
                        const Status = status;
                        ...
                        const finalURL = `/SystemStrengtheningSSNoActivity?data=${encodedData}`;
                        ...
                        return (
                          <StyledTableRow key={index}>
                            ...
                            <StyledTableCell>{element.D_NAME}</StyledTableCell>
                            <StyledTableCell>
                              {element.TALUKA_NAME}
                            </StyledTableCell>
                            <StyledTableCell>{element.SS_NO}</StyledTableCell>
                            <StyledTableCell>{element.S_NAME}</StyledTableCell>
                            <StyledTableCell>
                              {element.Approach_Taype}
                            </StyledTableCell>
                            <StyledTableCell>
                              {element.Percentage}
                            </StyledTableCell>
...
export default SystemStrengtheningSubstationActivityStatus;
