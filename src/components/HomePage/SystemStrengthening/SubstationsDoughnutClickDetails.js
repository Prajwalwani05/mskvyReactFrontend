...
import SelectActionCard from "./Cards_third";
 ...
 useEffect(() => {
  ...
  const fetchData = async (selectedSpvName = null) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${apiUrl}/api/SystemStrengtheningSSNoDetails_New`,
        {
          params: {
            SS_No: decryptedObject.SS_No,
            summaryId: decryptedObject.summaryId,
            ReferenceNo: decryptedObject.ReferenceNo,
            approachType: decryptedObject.approachType,
            status: decryptedObject.sstatus,
      ...
      if (response.status === 200) {
        ...
        console.log("SystemStrengtheningSSNoDetails_New: ", response.data);
      } else {
  ...
  return (
    <>
      {loading ? (
        <Loader />
      ) : substationData?.Model?.systemStrengtheningSS_NoDetailLists?.length > 0 ? (
        <Box
          sx={{
            backgroundColor: "#FFF",
            margin: "10px",
        ...
        >
          ...
          <Box p={1} pb={2}>
            <Box pb={2}>
              <SelectActionCard data={substationData?.Model?.systemStrengtheningSSCardDetailLists} spvName={currentSpvName} />
            </Box>
            <Box sx={{ width: "100%" }}>
...
export default SSDoughnutClickDetails;
