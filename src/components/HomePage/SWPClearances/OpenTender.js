...
const OpenTender = () => {
  ...
  const [
    openTenderSWPClearanceSummaryData,
    setOpenTenderSWPClearanceSummaryData,
  ...
  const fetchData = () => {
    axios
      .get(`${apiUrl}/api/OpenTenderSWPClearanceSummaryData?sfile_id=1`)
      .then((response) => {
        if (response.status === 200) {
          console.log("OpenTenderSWPClearanceSummaryData>>>", response.data);
          setOpenTenderSWPClearanceSummaryData(
  ...
  const totalPages = Math.ceil(
    (openTenderSWPClearanceSummaryData?.ClearanceSummaries?.length || 0) /
      itemsPerPage
  );
  ...
  const chartData =
    openTenderSWPClearanceSummaryData?.SummarySubstations?.map((ele) => ({
      label: ele.legendText || "Unknown", // Provide a fallback label if needed
      value: ele.y || 0, // Ensure there's a numeric value
    })) || [];
  //   const currentItems =
  //     openTenderSWPClearanceSummaryData?.ClearanceSummaries?.slice(
  //       indexOfFirstItem,
  //       indexOfLastItem
  //     ) || [];
        ...
        >
          ...
          >
            ...
            <h2 style={{ margin: "15px 0" }}>
              SWP Clearance Summary (Open Tender Approach)
            </h2>
            ...
            <Box sx={{ width: "100%" }}>
              <TableContainer sx={{ maxHeight: 620 }}>
                ...
                >
                  ...
                  <TableBody>
                    {openTenderSWPClearanceSummaryData?.ClearanceSummaries.map(
                      (element, index) => {
                        const spvNames =
                          openTenderSWPClearanceSummaryData?.ClearanceSummaries.map(
                            (ele) => ({
                              name: ele.SPV_Name_ProjectBase,
                              projectCode: ele.PROJECT_CODE,
                            })
                          ).filter((ele) => ele.name); // Removes empty names
                        // const encodedSpvNames = encodeURIComponent(spvNames.join(","));

                        const projectCode = element.PROJECT_CODE;
                        const sfile_id = "1";
                        const approachType = "Project";
                        const spvName = element.SPV_Name_ProjectBase;
                        ...
                        return (
                          <StyledTableRow key={index}>
                            ...
                            <StyledTableCell>
                              {element.Project_Solar_Capacity_MW}
                            </StyledTableCell>
