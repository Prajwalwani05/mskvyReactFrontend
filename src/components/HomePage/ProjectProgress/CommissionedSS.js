...
const CommissionedSS = () => {
  const [CommissionedDetails, setCommissionedDetails] = useState([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        setCommissionedDetails(response.data);
      } else {
  ...
  CommissionedDetails?.Model?._progressStatusCount?.filter((item) => (
      item.INVESTOR_CAPAPCITY !== 0
    )) || [];
      ...
      {filteredProgressData?.length > 0 ? (
        ...
        >
          ...
          >
            ...
            ></Box>
            <h2 style={{ margin: "15px 0" }}>Commissioned Substation List : All</h2>
                ...
                >
                  <TableHead>
                    <StyledTableRow>
                      ...
                      <StyledTableCell>SS Code</StyledTableCell>
                      <StyledTableCell>SS Name</StyledTableCell>
                      <StyledTableCell>Solar Capacity(MW)</StyledTableCell>
                      <StyledTableCell>Commissioned Capacity</StyledTableCell>
                    ...
                    {filteredProgressData?.map((ele, index) => (
                      <React.Fragment key={index}>
                        <StyledTableRow>
                          ...
                          <StyledTableCell>{ele.Approche_Type}</StyledTableCell>
                          <StyledTableCell>{ele.PPA_Reference_No}</StyledTableCell>
                          ...
                          <StyledTableCell>{ele.Subdivision_Name}</StyledTableCell>
                          <StyledTableCell>{ele.Solar_Capacity}</StyledTableCell>
                          <StyledTableCell>{ele.INVESTOR_CAPAPCITY}</StyledTableCell>
                        </StyledTableRow>
                      </React.Fragment>
                    ))}
                  </TableBody>
