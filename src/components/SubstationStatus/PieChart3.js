...
export default function PieClickNoSnap3(props) {
  ...
  const handleItemClick = (event, item) => {
    ...
    axios
      .get(
        `${apiUrl}/api/SubstationApi/GetTenderSubStationStatus?dist_code=${Dist_Code}&status1=Completed&clusterNo=${Dist_Code}`,
        {
      ...
      .then((response) => {
        if (response.status === 200) {
          setLoading(true);
          console.log("GetTenderSubStationStatus>>>", response.data);
