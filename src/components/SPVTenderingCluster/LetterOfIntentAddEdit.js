...
const LetterOfIntentAddEdit = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(
          `${apiUrl}/api/LetterOfIndentAddEdit`,
          {
            params: {
              v_loiID: decryptedObject.v_loiID,
              sfile_id: 1,
  ...
  useEffect(() => {
    ...
    if (letterOfIntentData && Object.keys(formData).length === 0) {
      setFormData(letterOfIntentData);
    }
  }, [letterOfIntentData, formData]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(
        `${apiUrl}/api/InsertUpdateLetterOfIndent`,
        formData
      );
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Grid container spacing={3} mb={1}>
              <Grid item xs={12} sm={3}>
                <TextField
                  label="Method"
                  name="IsE_Reverse_Or_DirectAward"
                  value={formData.IsE_Reverse_Or_DirectAward || ""}
                  onChange={handleChange}
                  size="medium"
                  fullWidth
                  disabled={isViewMode}
              ...
              <Grid item xs={12} sm={3}>
                <TextField
                  label="Tariff"
                  name="tariff_discovered"
                  value={formData.tariff_discovered || ""}
                  onChange={handleChange}
                  size="medium"
                  fullWidth
                  disabled={isViewMode}
            ...
            <Grid container spacing={3} mb={1}>
              <Grid item xs={12} sm={3}>
                <TextField
                  label="Scheduled LOI Issuance Date"
                  name="Scheduled_LOI_Issuance_Date"
                  value={formData.Scheduled_LOI_Issuance_Date || ""}
                  onChange={handleChange}
                  size="medium"
                  fullWidth
                  disabled={isViewMode}
              ...
              <Grid item xs={12} sm={3}>
                <TextField
                  label="Actual LOI Issuance_Date"
                  name="Actual_LOI_Issuance_Date"
                  value={formData.Actual_LOI_Issuance_Date || ""}
                  onChange={handleChange}
                  size="medium"
                  fullWidth
                  disabled={isViewMode}
            ...
            >
              <Button
                ...
                style={{color:'#069465'}}
                type="submit"
                onClick={() => navigate("/letterOfIntent")}
              >
                Back
              </Button>
              <Button
